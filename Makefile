...
wget http://vault.centos.org/3.1/os/x86_64/RedHat/RPMS/setarch-1.3-1.x86_64.rpm && \
setarch i386 rpm --root /chroot -ivh --nodeps centos-release*rpm && \
setarch i386 rpm --root /chroot -ivh --nodeps setarch*rpm && \
mkdir /chroot/proc && \
mkdir /chroot/sys && \
mkdir /chroot/dev && \
...
run: clean-containers build
	...
	docker run --rm $(IMAGE) sh -c 'setarch i386 yum -y install ruby && ruby -v'
